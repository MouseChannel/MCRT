...
{
    set->AddImageDescriptorTarget(m_hdr_img, (int)IBL_Binding::e_hdr_image, vk::ShaderStageFlagBits::eCompute, vk::DescriptorType::eCombinedImageSampler);

    set->AddImageDescriptorTarget(m_skybox_img,
                                  (int)IBL_Binding::e_skybox,
                                  vk::ShaderStageFlagBits::eCompute,
                                  vk::DescriptorType::eStorageImage);

    if (set->check_dirty()) {
        set_pool.reset(new DescriptorPool({ set }));
        set->build(set_pool, 1);
    }
    m_skyboxPass.reset(new ComputePass({ set }, sizeof(PushContant_IBL), "include/Shader/PBR/IBL/hdr2cubemap.comp.spv"));
...
void IBLManager::filter_skybox_mipmap()
{
    ...
    set->AddImageDescriptorTarget(img_views,
                                  img_layouts,
    ...
    m_skyboxFilterPass.reset(new ComputePass({ set }, sizeof(PushContant_IBL), "include/Shader/PBR/IBL/skybox_mipmap.comp.spv"));
    ...
    //     m_skyboxFilterPass->AddImageDescriptorTarget(
    //         m_skybox_img,
    //         (int)IBL_Binding::e_skybox,
    //         vk::ShaderStageFlagBits::eCompute,
    //         vk::DescriptorType::eCombinedImageSampler);
...
void IBLManager::pre_compute_LUT()
{
    ...
    set->AddImageDescriptorTarget(LUT,
    ...
    m_LUTPass.reset(new ComputePass({ set }, sizeof(PushContant_IBL), compute_shader));
    CommandManager::ExecuteCmd(Context::Get_Singleton()->get_device()->Get_Graphic_queue(),
                               [&](vk::CommandBuffer& cmd) {
    ...
    //     m_LUTPass->AddImageDescriptorTarget(
    //         LUT,
    //         (int)IBL_Binding::e_LUT_image,
    //         vk::ShaderStageFlagBits::eCompute,
    //         vk::DescriptorType::eStorageImage);
...
void IBLManager::pre_compute_irradiance()
{
    set->AddImageDescriptorTarget(m_skybox_img,
                                  ...
                                  vk::ShaderStageFlagBits::eCompute,
                                  vk::DescriptorType::eCombinedImageSampler);
    set->AddImageDescriptorTarget(m_irradiance_img,
    ...
    m_irradiancePass.reset(new ComputePass({ set }, sizeof(PushContant_IBL), compute_shader));
    CommandManager::ExecuteCmd(Context::Get_Singleton()->get_device()->Get_Graphic_queue(),
                               [&](vk::CommandBuffer& cmd) {
    ...
    //     m_irradiancePass->AddImageDescriptorTarget(
    //         m_skybox_img,
    //         (int)IBL_Binding::e_skybox,
    //         vk::ShaderStageFlagBits::eCompute,
    //         vk::DescriptorType::eCombinedImageSampler);

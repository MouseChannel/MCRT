...
namespace MCRT {
RaytracingContext::RaytracingContext(std::shared_ptr<Device> device)
    : m_device(device)
{
}
...
// std::shared_ptr<Pipeline_base> RaytracingContext::get_pipeline()
// {
//     return m_rt_pipeline;
// }

std::shared_ptr<Image> RaytracingContext::get_out_image()
{
    return m_out_image;
}

void RaytracingContext::build_accelerate_structure()
{

    Mesh::update_accelerate_structure_data();
    for (auto obj : Mesh::all_meshs) {
...
void RaytracingContext::create_shader_bind_table()
{
...
// void RaytracingContext::prepare_descriptorset(std::function<void()> prepare_func)
// {
//     prepare_func();
...
// void RaytracingContext::prepare_pipeline(std::vector<std::shared_ptr<ShaderModule>> shader_modules,
...
void RaytracingContext::create_uniform_buffer()
{
    ...
    m_objs_address.resize(Mesh::all_meshs.size());
    for (auto& obj : Mesh::all_meshs) {
...
void RaytracingContext::prepare()
{
...
void RaytracingContext::prepare_pipeline()
{
    
}

void RaytracingContext::create_offscreen_image()
{
...
void RaytracingContext::post_prepare()
{
    create_shader_bind_table();
}

void RaytracingContext::re_create()
{
    create_offscreen_image();
}

void RaytracingContext::update_ubo(std::shared_ptr<CommandBuffer> cmd)
{
...
// void RaytracingContext::record_command(std::shared_ptr<CommandBuffer> cmd)
// {
//     update_ubo(cmd);
...
std::shared_ptr<CommandBuffer> RaytracingContext::BeginFrame()
{
    m_command_buffer->Begin(vk::CommandBufferUsageFlagBits::eOneTimeSubmit);
    return m_command_buffer;
}

void RaytracingContext::Submit()
{
...
void RaytracingContext::EndFrame()
{
    // m_command_buffer->End();
}

...
void BasePass::Prepare_DescriptorSet(std::function<void()> prepare)
{
    ...
    if (m_descriptorSet->check_dirty()) {
        ...
        for (auto& [key, value] : m_descriptorSetTargetMap) {
            for (int i = 0; i < value->get_descriptor_count(); i++) {
        ...
        std::transform(type_map.begin(),
                       ...
                       [&](auto item) {
                           return vk::DescriptorPoolSize()
                               .setDescriptorCount(item.second *get_DescriptorSetCount() )
                               .setType(item.first);
                       });
...
void BasePass::AddDescriptorTarget(std::shared_ptr<DescriptorTargetBase> target)
{

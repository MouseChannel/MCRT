...
void RenderPass::Build(std::vector<std::shared_ptr<GraphicPass>> subpasses, std::vector<vk::SubpassDependency> subpass_dependency)
{
    std::vector<vk::SubpassDescription> subpass_des(subpasses.size());
    std::transform(subpasses.begin(),
                   subpasses.end(),
                   subpass_des.begin(),
                   [](std::shared_ptr<GraphicPass> item) { return item->get_description(); });
    vk::RenderPassCreateInfo create_info;
    create_info.setAttachments(attachment_descriptions);
